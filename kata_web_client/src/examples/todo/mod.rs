...
use strum_macros::{EnumIter, ToString}; // Issue: https://github.com/rust-analyzer/rust-analyzer/pull/6143#issuecomment-714119369
...
// https://github.com/yewstack/yew/blob/v0.17/examples/todomvc/src/lib.rs
...
impl Component for Model {
    ...
    fn create(_: Self::Properties, link: ComponentLink<Self>) -> Self {
        let storage = StorageService::new(Area::Local).expect("storage was disabled by user");
        let entries = {
    ...
    fn update(&mut self, msg: Self::Message) -> ShouldRender {
        match msg {
            ...
            Msg::UpdateEdit(value) => {
                log::info!("Msg::UpdateEdit {:?}", value);
                self.state.edit_value = value;
            }
            Msg::Remove(idx) => {
                log::info!("Msg::Remove {:?}", idx);
                self.state.remove(idx);
            }
            Msg::SetFilter(filter) => {
                log::info!("Msg::SetFilter");
                self.state.filter = filter;
            }
            Msg::ToggleEdit(idx) => {
                log::info!("Msg::ToggleEdit {:?}", idx);
                self.state.edit_value = self.state.entries[idx].description.clone();
                self.state.clear_all_edit();
                self.state.toggle_edit(idx);
            }
            Msg::ToggleAll => {
                log::info!("Msg::ToggleAll");
                let status = !self.state.is_all_completed();
                self.state.toggle_all(status);
            }
            Msg::Toggle(idx) => {
                log::info!("Msg::Toggle {:?}", idx);
                self.state.toggle(idx);
            }
            Msg::ClearCompleted => {
                log::info!("Msg::ClearCompleted");
                    ...
                    <section class=("main", hidden_class)>
                        ...
                        <ul class="todo-list">
                            { for self.state.entries.iter()
                                    .filter(|entry| self.state.filter.filter(entry)).enumerate()
        ...
        html! {
            <li class=class>
                <div class="view">
                    ...
                    <label ondblclick=self.link.callback(move |_| Msg::ToggleEdit(idx))>
                        { &entry.description }
                    </label>
...
impl State {
    ...
    fn total_completed(&self) -> usize {
        self.entries
            .iter()
            .filter(|entry| Filter::Completed.filter(entry))
            .count()
    }
    ...
    fn is_all_completed(&self) -> bool {
        let mut filtered_iter = self
            .entries
            .iter()
            .filter(|entry| self.filter.filter(entry))
            .peekable();
    ...
    fn clear_completed(&mut self) {
        let entries = self
            .entries
            .drain(..)
            .filter(|entry| Filter::Active.filter(entry))
            .collect();
    ...
    fn remove(&mut self, idx: usize) {
        let idx = {
            ...
            let entries = self
                ...
                .iter()
                .enumerate()
                .filter(|&(_, entry)| filter.filter(entry))
                .collect::<Vec<_>>();
